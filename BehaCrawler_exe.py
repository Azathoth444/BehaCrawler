...
class bahaCrawler:
    ...
    def setValue(self, bsn, post, current_page, max_download_page):
        self.bsn = bsn                                      #巴哈版，60076是場外休憩區
        self.post = post                                    #Copy巴哈文章最後字串，例如:snA=4953068&tnum=1106 或 snA=5516855&tnum=40 
        self.current_page = int(current_page)               #本程式默認從文章第1頁開始下載，若要由第10頁開始下載，可以將此數值設置為10
        self.max_download_page = int(max_download_page)     #由第1頁開始計算的話，下載至第3頁圖片，請設置為3　；　如果打算由第10頁下載至第30頁的話，請將此數值設置為30
...
if __name__ == "__main__":
    ...
    bsn = 60076
    bsn = input("請輸入巴哈bsn碼(場外為60076): ")                        #巴哈版，60076是場外休憩區
    post = input("請複製文章網址最後字串(例如: snA=4953068&tnum=1106): ") #Copy巴哈文章最後字串，例如:snA=4953068&tnum=1106 或 snA=5516855&tnum=40 

    current_page = 1
    current_page = input("打算從第幾頁開始下載(預設為1): ")               #本程式默認從文章第1頁開始下載，若要由第10頁開始下載，可以將此數值設置為10

    max_download_page = 1                                       #由第1頁開始計算的話，下載至第3頁圖片，請設置為3　；　如果打算由第10頁下載至第30頁的話，請將此數值設置為30
    max_download_page = input("打算下載至第幾頁內容(預設為1): ")

    print("\n")
    
    try:
        main.setValue(bsn, post, current_page, max_download_page)
        main.begin()
        main.createSoup()
    except Exception:
        print("有部分資料輸入錯誤")
